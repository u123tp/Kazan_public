...
package COMMON_FUNCS;
  ...
  function automatic logic is_write_policy(input req_policy_e req_policy_origin);
    if (req_policy_origin inside {WRITE_BYTE, WRITE_HALF_WORD, WRITE_WORD, WRITE_DOUBLE_WORD, WRITE_QUAD_WORD}) begin
      return 1;
    end else begin
      return 0;
    end
  ...
  function automatic logic is_read_policy(input req_policy_e req_policy_origin);
    if (req_policy_origin inside {READ_BYTE, READ_HALF_WORD, READ_WORD, READ_DOUBLE_WORD, READ_QUAD_WORD}) begin
      return 1;
    end else begin
      return 0;
    end
  ...
  // opcode_typeを取得する
  // opcode_typeを取得する
  function automatic opcode_type_e get_opcode_type(opcode_t opcode);
    case (opcode)

      // -------------------------------
      // 算術系（整数算術 + 即値算術 + LUI/AUIPC + M拡張）
      // -------------------------------
      NOP,
    ADD,  SUB,  SLL,  SLT,  OR,   AND,  XOR,  SRL,  SRA,  SLTU,
    ADDW, SUBW, SLLW, SRLW, SRAW,
    ADDI, SLTI, SLTIU, XORI, ORI, ANDI, SLLI, SRLI, SRAI,
    SRAIW, ADDIW, SLLIW, SRLIW,
    LUI, AUIPC,
    MUL, MULH, MULHSU, MULHU, 
    MULW, 
    BEQ, BNE, BLT, BGE, BLTU, BGEU,
    JAL, JALR:begin
        get_opcode_type = ARITH_OPCODE;
      end

      // div
      DIV, DIVU, REM, REMU, DIVW, DIVUW, REMW, REMUW: begin
        get_opcode_type = DIV_OPCODE;
      end

      // -------------------------------
      // ロード
      // -------------------------------
      LB, LH, LW, LD, LBU, LHU, LWU, LR_W, LR_D: begin
        get_opcode_type = LD_OPCODE;
      end
      ...
      // -------------------------------
      SB, SH, SW, SD,
    SC_W, AMOSWAP_W, AMOADD_W, AMOXOR_W, AMOAND_W, AMOOR_W,
    AMOMIN_W, AMOMAX_W, AMOMINU_W, AMOMAXU_W,
    SC_D, AMOSWAP_D, AMOADD_D, AMOXOR_D, AMOAND_D, AMOOR_D,
    AMOMIN_D, AMOMAX_D, AMOMINU_D, AMOMAXU_D:begin
        get_opcode_type = STAMO_OPCODE;
      end
    ...
    SFENCE_VMA,
    ILLEGAL_INSTR:begin
        $display("[ERROR][COMMON_FUNCS.svh] error in get_opcode_type() ");
        get_opcode_type = NONE_OPCODE;
      end
