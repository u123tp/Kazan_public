...
(
    ...
    input vuop_front_t op_in_div_cur[NUM_OF_DIV_ISSUE],
    ...
    output vuop_back_t op_out_div[NUM_OF_DIV_ISSUE],
  ...
  // stall_in_div
  // -------------------------------------------------
  vuop_front_t op_in_div[NUM_OF_DIV_ISSUE];

  vuop_front_t op_in_div_pending[NUM_OF_DIV_ISSUE];
  logic is_div_pending;

  vuop_front_t op_in_div_pending_next[NUM_OF_DIV_ISSUE];
  logic is_div_pending_next;

  always_comb begin
    is_div_pending_next = is_div_pending;
    op_in_div_pending_next = op_in_div_pending;

    if (flush_in) begin
      op_in_div           = '{default: 0};
      is_div_pending_next = 0;
    end else if (stall_in_from_div && !is_div_pending) begin
      // 新たにstallした場合.
      op_in_div_pending_next = op_in_div_cur;
      is_div_pending_next    = 1;
      op_in_div              = '{default: 0};
    end else if (stall_in_from_div) begin
      // 継続でstallした場合.
      op_in_div = '{default: 0};
    end else if (is_div_pending) begin
      // stall終了時.
      op_in_div           = op_in_div_pending;
      is_div_pending_next = 0;
    end else begin
      // 通常時.
      op_in_div = op_in_div_cur;

    end
  end

  always_comb begin
    stall_out_div = stall_in_from_div;
  end
  ...
  //パラメータ
  // --------------------------------------------------
  localparam int unsigned NUM_OF_PHYREG_READ_PORT = NUM_OF_ARI_ISSUE * 2 +NUM_OF_DIV_ISSUE*2 +NUM_OF_LD_ISSUE +NUM_OF_ST_AMO_ISSUE*2+NUM_OF_CSR_ISSUE;
  ...
  vuop_front_t op_in_div_prev[NUM_OF_DIV_ISSUE];
  ...
  always_comb begin
    ...
    for (int unsigned i = 0; i < NUM_OF_DIV_ISSUE; i++) begin
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+i*2] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+i*2] = op_in_div[i].rs1;
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+i*2+1] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+i*2+1] = op_in_div[i].rs2;
    end
    for (int unsigned i = 0; i < NUM_OF_LD_ISSUE; i++) begin
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+i] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+i] = op_in_ld[i].rs1;
      /* phyreg_r_en[NUM_OF_ARI_ISSUE*2+i*2+1] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+i*2+1] = op_in_ld[i].rs2; */
    end
    for (int unsigned i = 0; i < NUM_OF_ST_AMO_ISSUE; i++) begin
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2] = op_in_st_amo[i].rs1;
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2+1] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2+1] = op_in_st_amo[i].rs2;
    end
    for (int unsigned i = 0; i < NUM_OF_CSR_ISSUE; i++) begin
      phyreg_r_en[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+NUM_OF_ST_AMO_ISSUE*2+i] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+NUM_OF_ST_AMO_ISSUE*2+i] = op_in_csr[i].rs1;
      /* phyreg_r_en[NUM_OF_ARI_ISSUE*2+NUM_OF_LD_ISSUE+NUM_OF_ST_AMO_ISSUE*2+i*2+1] = 1;
      phyreg_r_ram_idx[NUM_OF_ARI_ISSUE*2+NUM_OF_LD_ISSUE+NUM_OF_ST_AMO_ISSUE*2+i*2+1] = op_in_csr[i].rs2; */
    end
  end
  ...
  always_comb begin
    ...
    for (int unsigned i = 0; i < NUM_OF_DIV_ISSUE; i++) begin
      op_out_div[i] = '{
          valid: op_in_div_prev[i].valid,
          opcode: op_in_div_prev[i].opcode,
          op_addr: op_in_div_prev[i].op_addr,
          rs1_exist: op_in_div_prev[i].rs1_exist,
          rs2_exist: op_in_div_prev[i].rs2_exist,
          rd_exist: op_in_div_prev[i].rd_exist,
          rs1: op_in_div_prev[i].rs1,
          rs2: op_in_div_prev[i].rs2,
          rd: op_in_div_prev[i].rd,
          rs1_val: phyreg_r_data[NUM_OF_ARI_ISSUE*2+i*2],
          rs2_val: phyreg_r_data[NUM_OF_ARI_ISSUE*2+i*2+1],
          imm: op_in_div_prev[i].imm,
          al_idx: op_in_div_prev[i].al_idx,
          actual_taken: 0,
          actual_taken_addr: 0
      };
    end
    ...
    for (int unsigned i = 0; i < NUM_OF_LD_ISSUE; i++) begin
      op_out_ld[i] = '{
          ...
          rs1_val: phyreg_r_data[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+i],
          rs2_val: 0,
    ...
    for (int unsigned i = 0; i < NUM_OF_ST_AMO_ISSUE; i++) begin
      op_out_st_amo[i] = '{
          ...
          rs1_val: phyreg_r_data[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2],
          rs2_val: phyreg_r_data[NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+i*2+1],
    ...
    for (int unsigned i = 0; i < NUM_OF_CSR_ISSUE; i++) begin
      op_out_csr[i] = '{
          ...
          rs1_val:
          phyreg_r_data[
          NUM_OF_ARI_ISSUE*2+NUM_OF_DIV_ISSUE*2+NUM_OF_LD_ISSUE+NUM_OF_ST_AMO_ISSUE*2+i
          ],
          rs2_val: 0,
  ...
  always_comb begin
    for (int unsigned i = 0; i < NUM_OF_RESULT_FEEDBACKS; i++) begin
      if (op_feedback[i].valid && op_feedback[i].rd_exist && op_feedback[i].rd != 0) begin
        phyreg_w_en[i] = 1;
...
always_ff @(posedge clock) begin
  if (reset) begin
    ...
    // stall状態更新
    is_div_pending  <= is_div_pending_next;
    is_ldst_pending <= is_ldst_pending_next;
    is_csr_pending  <= is_csr_pending_next;

    // pending命令更新
    op_in_div_pending    <= op_in_div_pending_next;
    op_in_ld_pending     <= op_in_ld_pending_next;
    op_in_st_amo_pending <= op_in_st_amo_pending_next;
    op_in_csr_pending    <= op_in_csr_pending_next;

    // prevの保存
    op_in_ari_prev    <= op_in_ari;
    op_in_div_prev    <= op_in_div;
  ...
  //   end else begin
  //     for (int unsigned i = 0; i < NUM_OF_LD_ISSUE; i++) begin
  //       if (op_in_ld[i].valid && op_in_ld[i].op_addr == 39'h80001018) begin
  //         $display("[LOG][INTPRF.sv] 80001018 op_ld_in rs1_phyreg:%h  rs2_phyreg:%h rd_phyreg:%h",
  //                  op_in_ld[i].rs1, op_out_ld[i].rs2, op_out_ld[i].rd);
  //       end
  //     end
  //     for (int unsigned i = 0; i < NUM_OF_LD_ISSUE; i++) begin
  //       if (op_out_ld[i].valid && op_out_ld[i].op_addr == 39'h80001018) begin
  //         $display("[LOG][INTPRF.sv] 80001018 op_ld_out rs1_val:%h  rs2_val:%h",
  //                  op_out_ld[i].rs1_val, op_out_ld[i].rs2_val);
  //       end
  //     end
  //     for (int unsigned i = 0; i < NUM_OF_RESULT_FEEDBACKS; i++) begin
  //       if (op_feedback[i].valid && op_feedback[i].rd_exist && op_feedback[i].rd == 15) begin
  //         $display("[LOG][INTPRF.sv] write to p15 feedback op_vaddr:%h rd_val:%h",
  //                  op_feedback[i].op_addr, op_feedback[i].rd_val);
  //       end
  //     end
  //   end
  // end
