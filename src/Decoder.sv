...
#(
  ...
  always_comb begin
    for (int i = 0; i < NUM_OF_FETCH; i++) begin
      automatic logic [31:0] fdata = fetched_content[i].data;
  ...
  always_comb begin
    ...
    for (int i = 0; i < NUM_OF_FETCH; i++) begin
      // if (fetched_content[i].data == 0 && fetched_content[i].is_page_fault) begin
      //   $display("[LOG][Decoder.sv] data == 0 and pf");
      // end
      decoded_op_next[i] = '{
          valid       : 0,
          vaddr: vaddr[i],
