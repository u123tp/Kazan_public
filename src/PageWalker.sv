...
#(
    ...
    input cache_res64_t res_from_dcache,
  ...
  // cicularを避けるために必要.
  cache_req64_t req_to_dcache_next;
  ...
  always_comb begin
    ...
    automatic PTE_t pte2 = '{default: 0};
    automatic PTE_t pte1 = '{default: 0};
    automatic PTE_t pte0 = '{default: 0};

    automatic logic [PADDR_WIDTH-1:0] page_paddr = 0;
    ...
    unique case (state)
      ...
      BEFORE_REQEST_LEVEL2: begin
        if (!dcache_noroom) begin
          ...
          req_to_dcache_next = '{
              ...
              vaddr_db_aligned: 0,
              paddr_valid: 1,
              paddr_db_aligned: level2_pte_paddr_next,
              byte_in_db_idx: 0,
              satp: 0,
              data: 0
          };
      ...
      WAITING_RESPONSE_LEVEL2: begin
        if (res_from_dcache.valid) begin
          ...
          // FOR DEBUG
          // $display("[LOG][PageWalker.sv] pte2 raw      = 0x%016h", pte2);
          // $display(
          //     "[LOG][PageWalker.sv] pte2 flags    : V=%0d R=%0d W=%0d X=%0d U=%0d G=%0d A=%0d D=%0d",
          //     pte2.V, pte2.R, pte2.W, pte2.X, pte2.U, pte2.G, pte2.A, pte2.D);
          // $display("[LOG][PageWalker.sv] pte2 RSW      = 0x%0h", pte2.RSW);
          // $display("[LOG][PageWalker.sv] pte2 PPN      : PPN2=0x%0h PPN1=0x%0h PPN0=0x%0h",
          //          pte2.PPN2, pte2.PPN1, pte2.PPN0);
          // $display("[LOG][PageWalker.sv] pte2 N/PBMT   : N=%0d PBMT=0x%0h", pte2.N, pte2.PBMT);
          // $display("[LOG][PageWalker.sv] pte2 reserved0= 0x%0h", pte2.reserved0);
      ...
      BEFORE_REQUEST_LEVEL1: begin
        if (!dcache_noroom) begin
          req_to_dcache_next = '{
              ...
              vaddr_db_aligned: 0,
              paddr_valid: 1,
              paddr_db_aligned: level1_pte_paddr,
              byte_in_db_idx: 0,
              satp: 0,
              data: 0
          };
      ...
      WAITING_RESPONSE_LEVEL1: begin
        if (res_from_dcache.valid) begin
          ...
          // FOR DEBUG
          // $display("[LOG][PageWalker.sv] pte1 raw      = 0x%016h", pte1);
          // $display(
          //     "[LOG][PageWalker.sv] pte1 flags    : V=%0d R=%0d W=%0d X=%0d U=%0d G=%0d A=%0d D=%0d",
          //     pte1.V, pte1.R, pte1.W, pte1.X, pte1.U, pte1.G, pte1.A, pte1.D);
          // $display("[LOG][PageWalker.sv] pte1 RSW      = 0x%0h", pte1.RSW);
          // $display("[LOG][PageWalker.sv] pte1 PPN      : PPN2=0x%0h PPN1=0x%0h PPN0=0x%0h",
          //          pte1.PPN2, pte1.PPN1, pte1.PPN0);
          // $display("[LOG][PageWalker.sv] pte1 N/PBMT   : N=%0d PBMT=0x%0h", pte1.N, pte1.PBMT);
          // $display("[LOG][PageWalker.sv] pte1 reserved0= 0x%0h", pte1.reserved0);
      ...
      BEFORE_REQUEST_LEVEL0: begin
        if (!dcache_noroom) begin
          req_to_dcache_next = '{
              ...
              vaddr_db_aligned: 0,
              paddr_valid: 1,
              paddr_db_aligned: level0_pte_paddr,
              byte_in_db_idx: 0,
              satp: 0,
              data: 0
          };
      ...
      WAITING_RESPONSE_LEVEL0: begin
        if (res_from_dcache.valid) begin
          ...
          // FOR DEBUG
          // $display("[LOG][PageWalker.sv] pte0 raw      = 0x%016h", pte0);
          // $display(
          //     "[LOG][PageWalker.sv] pte0 flags    : V=%0d R=%0d W=%0d X=%0d U=%0d G=%0d A=%0d D=%0d",
          //     pte0.V, pte0.R, pte0.W, pte0.X, pte0.U, pte0.G, pte0.A, pte0.D);
          // $display("[LOG][PageWalker.sv] pte0 RSW      = 0x%0h", pte0.RSW);
          // $display("[LOG][PageWalker.sv] pte0 PPN      : PPN2=0x%0h PPN1=0x%0h PPN0=0x%0h",
          //          pte0.PPN2, pte0.PPN1, pte0.PPN0);
          // $display("[LOG][PageWalker.sv] pte0 N/PBMT   : N=%0d PBMT=0x%0h", pte0.N, pte0.PBMT);
          // $display("[LOG][PageWalker.sv] pte0 reserved0= 0x%0h", pte0.reserved0);
