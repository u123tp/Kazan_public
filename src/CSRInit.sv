...
module CSRInit
  ...
  import CSR_PROPS::*;
(
    output csr_reg_u csr_regs_init[64]
);



  always_comb begin
    automatic misa_t misa_init = '{MXL: 2'b10, A: 1'b1, M: 1'b1, default: 0};
    automatic mstatus_t mstatus_init = '{UXL: 2'b10, SXL: 2'b10, default: 0};

    // counter
    automatic mcounteren_t mcounteren_init = '{IR: 1, TM: 1, CY: 1, default: 0};
    automatic scounteren_t scounteren_init = '{IR: 1, TM: 1, CY: 1, default: 0};
    automatic mcountinhibit_t mcountinhibit_init = '{default: 0};

    // menvcfg
    // Smcdeleg拡張をつける.
    automatic menvcfg_t menvcfg_init = '{CDE: 1, default: 0};

    // pmp
    // pmpの一番最初のエントリをtorにして,pmpaddr0の範囲をメインメモリ全体にする
    // こうすることで、起動時にpmpによるプロテクトを回避する.
    automatic pmpcfg_t pmpcfg0_init = {{56{1'b0}}, 8'b0_00_01_111};
    automatic pmpaddr_t pmpaddr0_init = (16 * 1024 * 1024) >> 2;

    // stimecmp
    // 設定し直さないかぎり割り込みが起こらないように最大値に.
    automatic stimecmp_t stimecmp_init = 64'hffff_ffff_ffff_ffff;
