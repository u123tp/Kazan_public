...
(
  ...
  // パラメータ
  // --------------------------------------------------
  localparam int unsigned NUM_OF_BTB_ENTRIES = 64;
  ...
  program_counter_t pc_in_prev[NUM_OF_FETCH];

  // addr_inの入力.
  always_comb begin
    for (int unsigned i = 0; i < NUM_OF_FETCH; i++) begin
      btb_r_en[i] = 1;
      btb_r_ram_idx[i] = pc_in[i].vaddr[BTB_IDX_MSB:BTB_IDX_LSB];
    end
  end
  ...
  always_comb begin
    ...
    addr_out = '{default: 0};
    for (int unsigned i = 0; i < NUM_OF_FETCH; i++) begin
      if (btb_r_data[i].valid && pc_in_prev[i].vaddr[TAG_MSB:TAG_LSB] == btb_r_data[i].tag) begin
        hit[i] = 1;
