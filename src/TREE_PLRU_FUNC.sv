...
//      3(1)  2(0)    * 1st layer
//     /  |  |   \    *
//    3   2  1    0   * result layer
//
// 1-wayの場合は常に0を返せば良い.
//         1(0)      * 0st layer = result_layer
...
interface TREE_PLRU_FUNC #(
  ...
  localparam int unsigned WAYS_IDX_WIDTH = NUM_OF_WAYS == 1 ? 1 : $clog2(NUM_OF_WAYS);
  localparam int unsigned NUM_OF_LAYERS = NUM_OF_WAYS == 1 ? 1 : $clog2(NUM_OF_WAYS);
  // localparam int unsigned LAYER_IDX_WIDTH = $clog2(NUM_OF_LAYERS);

  localparam int unsigned IDX_IN_LAYER_WIDTH = NUM_OF_WAYS == 1 ? 1 : NUM_OF_WAYS / 2;
  ...
  function automatic logic [WAYS_IDX_WIDTH-1:0] search_oldest(
    ...
    if (NUM_OF_WAYS == 1) begin
      return {WAYS_IDX_WIDTH{1'b0}};
    end else begin

      automatic logic [IDX_IN_LAYER_WIDTH-1:0] idx_in_layer_current = 0;
      automatic logic [IDX_IN_LAYER_WIDTH-1:0] idx_in_layer_next = 0;
      automatic structured_idx_t structured_idx;

      for (int unsigned current_layer = 0; current_layer < NUM_OF_LAYERS; current_layer++) begin
        idx_in_layer_current = idx_in_layer_next;
        structured_idx = '{layer_idx: current_layer, idx_in_layer: idx_in_layer_current};
        if (plru_tree_tmp[idx_structured_to_raw(structured_idx)]) begin
          // 1なら左に進む.
          idx_in_layer_next = IDX_IN_LAYER_WIDTH'(2 * idx_in_layer_current + 1);
        end else begin
          // 0なら右に進む.
          idx_in_layer_next = IDX_IN_LAYER_WIDTH'(2 * idx_in_layer_current);
        end
      end
  ...
  function automatic logic [NUM_OF_WAYS-1:0] update_plru_tree(
    ...
    if (NUM_OF_WAYS == 1) begin
      return {NUM_OF_WAYS{1'b0}};
    end else begin

      // plru_bool_log[i]はレイヤーiにおける経路上の1,0を記録.
      automatic logic plru_bool_log[$clog2(NUM_OF_WAYS)];
      automatic logic [NUM_OF_WAYS-1:0] plru_tree_new = plru_tree_tmp;
      ...
      begin
        automatic logic [WAYS_IDX_WIDTH-1:0] raw_idx;
        automatic structured_idx_t structured_idx;
        automatic logic [IDX_IN_LAYER_WIDTH-1:0] idx_in_layer_current = 0;
        automatic logic [IDX_IN_LAYER_WIDTH-1:0] idx_in_layer_next = 0;
